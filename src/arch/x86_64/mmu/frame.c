...
void frame_init(multiboot_info_t* mbi)
{
    ...
    printf("Initialized frame allocator with start %p and end %p "
            "mamax_allocatable_frames=%llu allocated_frames=%p used_count=%llu "
       "first_request=%llu\n",
...
opt_uint64_t frame_allocate()
{
  ...
  if (request == 0) {
    ...
    printf("ERROR: %s", "no more allocatable frames\n");
  ...
  if (frame.has_value) {
    printf("allocated frame: addr=%p request=%llu\n", frame.value, request);
    bitmap_set(allocated_frames, request);
  } else {
    printf("could not allocate frame: request=%llu\n", request);
  }
